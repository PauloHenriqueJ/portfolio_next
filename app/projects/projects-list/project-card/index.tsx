...
export const ProjectCard = ({project}: ProjectCardProps) => {
    ...
    return(
        <div className="rounded-lg h-[436px] flex flex-col bg-gray-800 overflow-hidden border-2 border-gray-800 hover:border-emerald-500
        opacity-70 hover:opacity-100 transition-all group">
            <div className="w-full h-48">
                ...
                alt={`webcarros ${project.title}`}
                unoptimized
                className="w-full h-full object-cover group-hover:scale-110 duration-500 transition-all"
                />

                <div className="fle-1 flex flex-col p-8">
        ...
        <s className="text-gray-300 text-sm font-medium block mt-auto truncate">{technologies}</s>
                </div>
            </div>
        </div>
    )
